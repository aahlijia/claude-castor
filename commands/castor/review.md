...
`gemini_review` returns a correctness-focused review (bugs, broken edge cases, error handling, boundary and concurrency mistakes), not style nitpicks. It is a cheap extra pass that complements — does not replace — your own review. It runs agy's `code-reviewer` agent by default (pass `agent=None` to fall back to agy's default agent, or another name to override). For a security-focused pass on the same diff, use `/castor:security-review` instead.
