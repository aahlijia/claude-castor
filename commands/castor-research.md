Research a specific topic, feature, symbol, or file in the codebase using Antigravity (agy) in agent mode.
...
- No `files` or `directory` — let agy locate what it needs
...
After agy responds, synthesize the findings into directly actionable context for the current task. Verify any specific line numbers or function signatures before acting on them.
